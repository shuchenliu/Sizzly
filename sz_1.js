...
var result = function getSZ(code, inputKeyword, docType, startDate, endDate) {
     ...
     function keepGoing(formData, contentLength) {
       request({
          ...
          for (var j = 0; j < target.length  - 2; ++j) {
            ...
            //var content = pubDate +'\t' + form.stockCode + '\t' + iconv.decode(filename,'utf-16') +'\r\n';
            var content = [pubDate, form.stockCode, filename, '\r\n'].join(',');

            request.get(downloadURL).pipe(fs.createWriteStream(saveDir));
          ...
          if (currentPage != totalPage) {
            ...
            console.log('Please wait for all your docs to be downloaded before pressing control + c to procceed.');
          }
         });
     }
